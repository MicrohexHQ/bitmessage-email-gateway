...
class GWUser(object):
	...
	def add(self, bm, email, postmap = None):
		...
		filterwarnings('ignore', category = MySQLdb.Warning)
		cur.execute ("INSERT IGNORE INTO user (bm, email, postmap, domain, credits, exp, active, cansend, cancharge, caninvoice, attachments) VALUES (%s, %s, %s, %s, '0', '9999-12-31', 1, 1, 0, 0, 0)", (
			bm, email, postmap, domain))
		uid = None
		if cur.rowcount == 1:
			uid = cur.lastrowid
			self.load(uid = uid)
...
def adduser(bm, email, postmap = None):
	BMMySQL().db.ping(True)
	cur = BMMySQL().db.cursor()
	if postmap == None:
		postmap = pwd.getpwuid(os.getuid())[0]
...
def deluser(bm = None, email = None):
	BMMySQL().db.ping(True)
	cur = BMMySQL().db.cursor()
	filterwarnings('ignore', category = MySQLdb.Warning)
	if bm != None:
		cur.execute ("DELETE FROM user WHERE bm = %s", (bm))
		logging.info('Deleted user ' + bm)
	elif email != None:
...
def setparam(bm = None, email = None, lastrelay = None):
	BMMySQL().db.ping(True)
	cur = BMMySQL().db.cursor()
	filterwarnings('ignore', category = MySQLdb.Warning)
	...
	if bm != None:
		if lastrelay == None:
			cur.execute ("UPDATE user SET lastrelay = UNIX_TIMESTAMP() WHERE bm = %s", (bm))
		else:
			cur.execute ("UPDATE user SET lastrelay = %s WHERE bm = %s", (lastrelay, bm))
	elif email != None:
		if lastrelay == None:
			cur.execute ("UPDATE user SET lastrelay = UNIX_TIMESTAMP() WHERE email = %s", (email))
		else:
			cur.execute ("UPDATE user SET lastrelay = %s WHERE email = %s", (lastrelay, email))
	else:
...
def getuser(bm = None, email = None, unalias = False):
	BMMySQL().db.ping(True)
	cur = BMMySQL().db.cursor(MySQLdb.cursors.DictCursor)
	result = False
	filterwarnings('ignore', category = MySQLdb.Warning)
	multirow = False
	columns = ", ".join(["bm", "email", "postmap", "domain", "active", "cansend", "cancharge", "caninvoice", "attachments", "credits", "exp", "lastrelay", "masterpubkey_btc", "offset_btc", "feeamount", "feecurrency", "archive", "flags"])
	if bm != None:
		cur.execute ("SELECT " + columns + " FROM user WHERE bm = %s", (bm))
	elif email != None:
		if unalias:
			email = _unalias(email)
		cur.execute ("SELECT " + columns + " FROM user WHERE email = %s", (email))
	else:
		# if no arguments, return all
		multirow = True
		result = []
		cur.execute ("SELECT " + columns + " FROM user ORDER BY email")
...
def getdomain (domain = None):
	...
	filterwarnings('ignore', category = MySQLdb.Warning)
	multirow = False
	columns = ", ".join(["name", "active"])
	if domain != None:
		cur.execute ("SELECT " + columns + " FROM domain WHERE active = 1 AND name = %s", (domain))
	else:
		multirow = True
		result = []
		cur.execute ("SELECT " + columns + " FROM domain WHERE active = 1")
	for row in cur.fetchall():
		if multirow:
			result.append(row)
		else:
